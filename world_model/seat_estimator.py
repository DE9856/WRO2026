...
# ---------------------------------------------------------------------------
# Self-test — synthetic detections, no camera/hardware required.
# ---------------------------------------------------------------------------
def _run_tests():
    print("Running seat_estimator.py tests...")

    # --- Test 1: column thirds.
    assert classify_column(10, 300) == "LEFT"
    assert classify_column(150, 300) == "CENTER"
    assert classify_column(290, 300) == "RIGHT"
    print("  [PASS] classify_column splits ROI into LEFT/CENTER/RIGHT thirds")
    ...
    print("  [PASS] classify_row splits near/far at TOP_BOTTOM_SPLIT_CM")

    # --- Test 3: full seat classification for all 6 combinations.
    roi_w = 300
    cases = [
    ...
    print("  [PASS] classify_seat covers all 6 TL/TC/TR/BL/BC/BR combinations")

    # --- Test 4: classify_detections batches correctly and rejects bad color.
    ...
    try:
        ...
        raise AssertionError("classify_detections should reject an invalid color")
    except ValueError:
        pass
    print("  [PASS] classify_detections batches seats and validates color")

    print("All seat_estimator.py tests passed.")
