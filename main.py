...
from camera.corridor   import estimate_corridor, corridor_error
...
def main():
    ...
    # =====================================
    parking_maneuver = ParkingManeuver()
        ...
        while True:
            ...
            if closest_red:
                ...
                if err is not None:
                    ...
                    # No pillar AND no corridor reading this frame -- hold
                    # the last commanded steering rather than snapping
            ...
            if planner.should_execute_parking():
                park_steer, park_speed, centered = parking_maneuver.update(parking_lot, roi_width)
            ...
            # button in this software-only demo; no-op once past WAIT.
            # =====================================
            if not args.headless:
                key = cv2.waitKey(1)
                if key == ord(" "):
