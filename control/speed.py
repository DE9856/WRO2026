...
Output is a PWM-style duty value in [0, 100] for forward driving
...
# ---------------------------------------------------------------------------
# Self-test — synthetic angle/confidence/lap_phase combinations, no
# camera or hardware needed.
# ---------------------------------------------------------------------------
def _run_tests():
    print("Running speed.py tests...")

    # --- Test 1: straight-line, full confidence, SPEED_RUN -> near CRUISE_PWM.
    sc = SpeedController()
    pwm = sc.compute(steering_angle_deg=0, confidence=1.0, lap_phase="SPEED_RUN", smoothing=0.0)
    assert abs(pwm - CRUISE_PWM) < 1e-6, pwm
    print("  [PASS] straight line + full confidence + SPEED_RUN -> full CRUISE_PWM")

    # --- Test 2: sharp corner -> drops to CORNER_PWM tier.
    sc = SpeedController()
    pwm = sc.compute(steering_angle_deg=25, confidence=1.0, lap_phase="SPEED_RUN", smoothing=0.0)
    assert abs(pwm - CORNER_PWM) < 1e-6, pwm
    print("  [PASS] sharp steering angle drops PWM to the CORNER_PWM tier")

    # --- Test 3: low confidence throttles speed down even in a straight line.
    sc = SpeedController()
    ...
    print("  [PASS] low confidence reduces speed but never below MIN_PWM")

    # --- Test 4: lap_phase multiplier scales speed — MAP_BUILD is more cautious than SPEED_RUN.
    sc1 = SpeedController()
    ...
    print("  [PASS] MAP_BUILD lap phase drives slower than SPEED_RUN at identical angle/confidence")

    # --- Test 5: parking=True overrides the angle-based schedule with PARK_PWM.
    sc = SpeedController()
    ...
    print("  [PASS] parking=True overrides angle/confidence/lap_phase with flat PARK_PWM")

    # --- Test 6: smoothing dampens a sudden target change rather than jumping instantly.
    sc = SpeedController()
    ...
    print("  [PASS] smoothing dampens a sudden angle change instead of snapping instantly")

    # --- Test 7: reset() clears the smoothing memory.
    sc = SpeedController()
    sc.compute(steering_angle_deg=0, confidence=1.0, lap_phase="SPEED_RUN", smoothing=0.9)
    sc.reset()
    assert sc._last_pwm == 0.0
    print("  [PASS] reset() clears smoothing memory")

    print("All speed.py tests passed.")
